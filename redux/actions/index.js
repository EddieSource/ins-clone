...
const resetFollowingLoad = () => {
  return (dispatch) => {
    dispatch({ type: USERS_RESET_FOLLOW_LOAD })
  }
}
  ...
  return (dispatch, getState) => {
    const unsubscribe = firebase
      ...
      .onSnapshot((snapshot) => {
        ...
        const reduxUsersFollowingLoaded =
          getState().usersState.usersFollowingLoaded

        if (following.length < reduxUsersFollowingLoaded) {
          console.log('user is unfollowing')
          dispatch({ type: USERS_RESET_FOLLOW_LOAD, following })
        }
      })
    return unsubscribe
  }
...
export function fetchUsersData(uid, getPosts) {
  return (dispatch, getState) => {
    ...
    console.log('current usersState.users: ', uid)
